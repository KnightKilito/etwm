	...
	<div class="page-wrapper">
		...
		<section class="testimonials_one three">
			...
			<div class="container">
							...
							<div class="latest_listings_three_single wow fadeInUp"
								...
								<div class="latest_listings_three_content">
									<div class="title">
										...
										<c:if test="${username==null }">
											<h3>
												<a href="allshopproducts.do?shopId=${shop.id }">${shop.shopName }<span
													class="fa fa-check"></span></a>
											</h3>
										</c:if>
									</div>
				...
				<center>
					<c:if test="${username==null }">
					<a href="searchShop.do?pageNum=1&pageSize=${pageSize}&search=${searchName}"
					class="btn btn-default">第一页 </a>
				<c:if test="${currPage!=1 }">
					<a href="searchShop.do?pageNum=${currPage-1 }&pageSize=${pageSize}&search=${searchName}"
				...
				<a href="searchShop.do?pageNum=${allPage}&pageSize=${pageSize}&search=${searchName}"
				...
				<c:if test="${username!=null }">
					<a href="searchShop.do?username=${username }&pageNum=1&pageSize=${pageSize}&search=${searchName}"
					class="btn btn-default">第一页 </a>
				<c:if test="${currPage!=1 }">
					<a href="searchShop.do?username=${username }&pageNum=${currPage-1 }&pageSize=${pageSize}&search=${searchName}"
						class="btn btn-default">上一页</a>
				</c:if>
				共 ${allCount } 条，共 ${allPage } 页 ，当前在 ${currPage } 页
				<c:if test="${currPage!=allPage }">
				...
				<a href="searchShop.do?username=${username }&pageNum=${allPage}&pageSize=${pageSize}&search=${searchName}"
					class="btn btn-default">最后一页 </a>
				</c:if>	
				</center>
			</div>
		...
		<div class="search-popup__inner">
			<form action="searchShop.do?" class="search-popup__form">
				<input type="hidden" name="pageNum" value="1"> <input
					type="hidden" name="pageSize" value="8">
				<!-- 如果username为空就不传参了，免得无法判断为""的情况 -->
				<c:if test="${username!=null }">
