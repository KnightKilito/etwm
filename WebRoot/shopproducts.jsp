	...
	<div class="page-wrapper">
					...
					<div class="container-box clearfix">
						...
						<div class="main-nav__right main-nav__right_one three float-right">
							...
							<c:set var="kon" value=""></c:set>
								...
								<c:if test="${username!=null }">
									<a href="myaccount.ado" class="login"><span class="fas fa-user"></span>${username }</a>
								</c:if>
							</div>
							<div class="icon_cart_box">
								<a href="allshopcar.ado?username=${username }"> <span class="icon-shopping-cart"></span>
								</a>
							</div>
		...
		<section class="testimonials_one three">
            ...
            <div class="block-title text-center">
                    <h4>${shop.shopName } 's Products</h4>
                    <h2>Which One You Wanna Choose ?</h2>
                    
            </div>
			<div class="container">
				
				<div class="row">
					<c:forEach items="${list }" var="shopProduct">
                    ...
                    <div class="col-xl-3 col-lg-4 col-md-6">
                        ...
                        <div class="latest_listings_three_single wow fadeInUp" data-wow-delay="0ms"
                            ...
                            <div class="latest_listings_three_image">
                                <img src="assets/images/resources/shopproducts/${shop.shopName}/${shopProduct.productPicFile}" alt="">
                                
                                
                                <div class="shopping_circle">
                                    <!-- 添加产品到购物车 -->
                                    <a href="addtocar.ado?productId=${shopProduct.id }&shopId=${shop.id}&username=${username}" style="color:white"><span class="icon-shopping-bags"></span></a>
                                </div>
                            </div>
                            <div class="latest_listings_three_content">
                                <div class="title">
                                    <h3>${shopProduct.productName }<span
                                                class="fa fa-check"></span></h3>
                                </div>
                                <ul class="list-unstyled latest_listings_three_contact_info">
                                    <li><i class="fas fa-map-marker-alt"></i>${shopProduct.productIntroduction }</li>
                                   <%--  <li><a href="tel:+13456789"><i class="fa fa-phone"></i>${shop.connectPhone }</a></li> --%>
                                </ul>
                                <div class="latest_listings_three_content_bottom">
                                    <div class="left">
                                        <h6>¥ ${shopProduct.productPrice }</h6>
                                    </div>
                                    <div class="rating">
                                    	<a href="#" class="clr-black">${shopProduct.productStar }</a>
                                    	<c:forEach begin="1" end="${Math.floor(shopProduct.productStar) }" >
                                    		<a href="#"><i class="fa fa-star"></i></a>
                                    	</c:forEach>
                                    	
                                        <c:forEach begin="1" end="${5-Math.floor(shopProduct.productStar) }">
				...
				<center>
				<c:if test="${username==null }">
					<a href="allshopproducts.do?pageNum=1&pageSize=${pageSize}&search=${searchName}"
					class="btn btn-default">第一页 </a>
				<c:if test="${currPage!=1 }">
					<a href="allshopproducts.do?pageNum=${currPage-1 }&pageSize=${pageSize}&search=${searchName}"
				...
				<a href="allshopproducts.do?pageNum=${allPage}&pageSize=${pageSize}&search=${searchName}"
				...
				<c:if test="${username!=null }">
					<a href="allshopproducts.do?username=${username }&pageNum=1&pageSize=${pageSize}&search=${searchName}"
					class="btn btn-default">第一页 </a>
				<c:if test="${currPage!=1 }">
					<a href="allshopproducts.do?username=${username }&pageNum=${currPage-1 }&pageSize=${pageSize}&search=${searchName}"
						class="btn btn-default">上一页</a>
				</c:if>
				共 ${allCount } 条，共 ${allPage } 页 ，当前在 ${currPage } 页
				<c:if test="${currPage!=allPage }">
				...
				<a href="allshopproducts.do?username=${username }&pageNum=${allPage}&pageSize=${pageSize}&search=${searchName}"
					class="btn btn-default">最后一页 </a>
				</c:if>	
				</center>
			</div>
		...
		<div class="site-footer_bottom">
			<div class="container">
				<div class="site-footer_bottom_copyright">
					<p>
						Copyright &copy; 2021.Java Knights All rights reserved.
					</p>
				</div>
