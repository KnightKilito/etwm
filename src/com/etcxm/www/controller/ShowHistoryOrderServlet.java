...
/**
 * Servlet implementation class ShowHistoryOrderServlet
 */
@WebServlet("/showhistoryorder.ado")
public class ShowHistoryOrderServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<ShopProduct> list = shopCarProductService.findAllHistory(shopName);
		request.setAttribute("list", list);
		// 获取总共多少条记录
		int allCount = shopCarProductService.countHistory(shopName);
		request.setAttribute("allCount", allCount);
		request.getRequestDispatcher("view/historyshoporder.jsp").forward(request, response);
	}
